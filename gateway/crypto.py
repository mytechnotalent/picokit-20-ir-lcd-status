...
This module implements the authenticated field profile for the cold chain
monitor with only the Python standard library. It derives the shared
field key with RFC 9106 Argon2id and seals or opens compact telemetry
frames with XChaCha20-Poly1305 using the node identifier as associated
data. The byte layout interoperates with the RP2350 firmware.
...
AEAD_TAG_LEN = 16
FIELD_PASSWORD = b"picokit field key v1"
FIELD_SALT = b"picokit-salt-0001"[:16]
FIELD_TIME_COST = 3
...
def derive_field_key():
    """Derive and cache the cold chain monitor field key.
...
def xchacha_seal(key, nonce, aad, plaintext):
    """Seal plaintext bytes with XChaCha20-Poly1305.
    ...
    tuple of bytes
        Ciphertext and sixteen-byte authentication tag.
    """
    return _xchacha_seal(key, nonce, aad, plaintext)


def xchacha_open(key, nonce, aad, ciphertext, tag):
    """Authenticate and open an XChaCha20-Poly1305 frame.
    ...
    ValueError
        When the tag does not authenticate the frame.
    """
    return _xchacha_open(key, nonce, aad, ciphertext, tag)
