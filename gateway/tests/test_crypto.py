...
FRAME_CT = "e0f2ae2cf9907fb0c87847f9b6d5bb49ba7f5ce178"
FRAME_TAG = "69a6db1342ea6321ac49160ab7f993ac"
PLAINTEXT = b'{"n":20,"s":12,"g":1}'
...
def test_envelope_matches_firmware():
    ...
    ciphertext, tag = crypto._xchacha_seal(
        crypto.derive_field_key(), nonce, bytes([20]), PLAINTEXT)
...
def test_public_aead_round_trip():
    ...
    nonce = bytes(range(24))
    ciphertext, tag = crypto.xchacha_seal(key, nonce, b"\x14", PLAINTEXT)
    opened = crypto.xchacha_open(key, nonce, b"\x14", ciphertext, tag)
