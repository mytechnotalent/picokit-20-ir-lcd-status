 ...
 * the ChaCha20, Poly1305, XChaCha20-Poly1305, BLAKE2b, and Argon2id
 * security modules.
 *
 * BRIEF:
...
/**
 * @brief Tampered associated data used to reject a forged frame.
 */
...
static void assert_lamps(int red, int yellow, int green) {
    TEST_ASSERT_EQUAL_INT(red, mock_gpio_get(PICOKIT_20_IR_LCD_STATUS_RED_LED_PIN));
    TEST_ASSERT_EQUAL_INT(yellow,
                          mock_gpio_get(PICOKIT_20_IR_LCD_STATUS_YELLOW_LED_PIN));
    TEST_ASSERT_EQUAL_INT(green,
                          mock_gpio_get(PICOKIT_20_IR_LCD_STATUS_GREEN_LED_PIN));
}
...
static void test_status_led_init(void) {
    ...
    TEST_ASSERT_TRUE(s_mock_gpio_dirs[PICOKIT_20_IR_LCD_STATUS_RED_LED_PIN]);
    TEST_ASSERT_TRUE(s_mock_gpio_dirs[PICOKIT_20_IR_LCD_STATUS_YELLOW_LED_PIN]);
    TEST_ASSERT_TRUE(s_mock_gpio_dirs[PICOKIT_20_IR_LCD_STATUS_GREEN_LED_PIN]);
}
 ...
 */
static const uint8_t s_env_pt[] = "{\"n\":20,\"s\":0,\"g\":0}";

/**
 * @brief Single associated-data byte carrying the provisioned node identity.
 */
static const uint8_t s_env_ad = (uint8_t)PACKET_NODE_ID;
 ...
 */
static const char s_env_known_hex[] = "000102030405060708090a0b0c0d0e0f1011121314151617e5e0615daae0bd82113704f4fb7e8a8f697d18d8c546943441113750b051fdb859a5166f";
