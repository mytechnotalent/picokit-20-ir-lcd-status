/**
 * FILE: test_picokit_20_ir_lcd_status_and_security.c
 *
 * DESCRIPTION:
 * Native unit tests for the RP2350 Picokit infrared keypad node:
 * provisioning constants, the packet artifact, CRC, the RYLR998 AT
 * interface, the VS1838B NEC decoder, the 1602 I2C LCD renderer, and the
 * keypad state machine that pairs each accumulated value with an
 * authenticated heartbeat.
 *
 * BRIEF:
 * Native unit test runner for picokit-20-ir-lcd-status.
 *
 * AUTHOR: Kevin Thomas
 * DATE: September 2026
 */
...
#include "picokit_20_ir_lcd_status.h"
#include "crc.h"
#include "radio.h"
#include "ir_remote.h"
#include "sensor.h"
...
static void nec_arm_at(uint8_t command, uint64_t base) {
    ...
    mock_gpio_timeline_begin_at(base, s_ir_off, s_ir_lvl, count,
                                PICOKIT_20_IR_LCD_STATUS_IR_PIN);
}
...
static void assert_pin_constants(void) {
    TEST_ASSERT_EQUAL_UINT(25u, PICOKIT_20_IR_LCD_STATUS_LED_PIN);
    TEST_ASSERT_EQUAL_UINT(5u, PICOKIT_20_IR_LCD_STATUS_IR_PIN);
    TEST_ASSERT_EQUAL_UINT(8u, PICOKIT_20_IR_LCD_STATUS_UART_TX);
    TEST_ASSERT_EQUAL_UINT(9u, PICOKIT_20_IR_LCD_STATUS_UART_RX);
}
...
static void assert_frame_constants(void) {
    TEST_ASSERT_EQUAL_UINT(115200u, PICOKIT_20_IR_LCD_STATUS_UART_BAUD);
    TEST_ASSERT_EQUAL_UINT(48u, PICOKIT_20_IR_LCD_STATUS_FRAME_SIZE);
    TEST_ASSERT_EQUAL_UINT(5000u, PICOKIT_20_IR_LCD_STATUS_TX_INTERVAL_MS);
    TEST_ASSERT_EQUAL_UINT(200u, MONITOR_POLL_INTERVAL_MS);
}
...
static void assert_artifact_ids(void) {
    TEST_ASSERT_EQUAL_UINT(20u, PACKET_NODE_ID);
...
static void assert_artifact_example(void) {
    ...
    TEST_ASSERT_EQUAL_UINT8(0x32u, PACKET_EXAMPLE_FRAME[5]);
...
/**
 * @brief Arm a due transmit tick with a decoded NEC frame.
 *
 * @param void No parameters.
 * @return void
 */
static void arm_transmit(void) {
    init_monitor();
    mock_timer_set_us(6000000u);
    nec_arm_at(0x45u, 6000000u);
    step_at(6000000u);
}
...
void test_ir_init(void) {
    ...
    TEST_ASSERT_FALSE(s_mock_gpio_dirs[PICOKIT_20_IR_LCD_STATUS_IR_PIN]);
}
...
void test_ir_poll_stuck_high(void) {
    ir_command_t cmd;
    reset_all();
    gpio_put(PICOKIT_20_IR_LCD_STATUS_IR_PIN, true);
    TEST_ASSERT_FALSE(ir_remote_poll(&cmd));
}
...
void test_monitor_build_frame_json(void) {
    char frame[PICOKIT_20_IR_LCD_STATUS_FRAME_SIZE];
    g_seq = 12u;
    g_page = 1u;
    monitor_build_frame(frame, sizeof(frame));
    TEST_ASSERT_EQUAL_STRING("{\"n\":20,\"s\":12,\"g\":1}", frame);
}
...
void test_monitor_init(void) {
    ...
    TEST_ASSERT_TRUE(s_mock_gpio_dirs[PICOKIT_20_IR_LCD_STATUS_LED_PIN]);
...
void test_monitor_page_next(void) {
    init_monitor();
    page_step(0x45u, 1000000u);
    TEST_ASSERT_EQUAL_UINT8(1u, g_page);
    TEST_ASSERT_EQUAL_STRING("PAGE 2/3", g_line1);
    TEST_ASSERT_EQUAL_STRING("SENSOR N:20", g_line2);
}

void test_monitor_page_wrap(void) {
    init_monitor();
    page_step(0x46u, 1000000u);
    TEST_ASSERT_EQUAL_UINT8(2u, g_page);
    TEST_ASSERT_EQUAL_STRING("PAGE 3/3", g_line1);
    TEST_ASSERT_EQUAL_STRING("RADIO N:20", g_line2);
}

void test_monitor_page_ignore(void) {
    init_monitor();
    page_step(0x44u, 1000000u);
    TEST_ASSERT_EQUAL_UINT8(0u, g_page);
    TEST_ASSERT_EQUAL_UINT8(0x44u, g_key_code);
}

void test_monitor_poll_none(void) {
    init_monitor();
    step_at(mock_timer_now_us());
    TEST_ASSERT_EQUAL_UINT8(0x00u, g_key_code);
}

void test_display_init(void) {
    reset_all();
    TEST_ASSERT_TRUE(display_init(i2c1, PICOKIT_20_IR_LCD_STATUS_LCD_ADDR));
    TEST_ASSERT_TRUE(mock_i2c_log_count() > 0u);
}
...
void test_display_init_fails(void) {
    reset_all();
    mock_i2c_set_write_fail(true);
    TEST_ASSERT_FALSE(display_init(i2c1, PICOKIT_20_IR_LCD_STATUS_LCD_ADDR));
}
...
void test_display_format(void) {
    ...
    TEST_ASSERT_EQUAL_STRING("N:20 S:0042 OK", s_line2);
}
...
void test_display_format_negative(void) {
    ...
    TEST_ASSERT_EQUAL_STRING("N:20 S:0000 !!", s_line2);
}

void test_display_render(void) {
    reset_all();
    strcpy(s_line1, "PIN 0012");
    strcpy(s_line2, "D2 N:17");
    display_render_lines(i2c1, PICOKIT_20_IR_LCD_STATUS_LCD_ADDR, s_line1, s_line2);
...
void test_monitor_transmit_frame(void) {
    size_t tx_len;
    arm_transmit();
    TEST_ASSERT_EQUAL_UINT8(0x45u, g_key_code);
...
/**
 * @brief Run the infrared poll monitor state-machine tests.
 *
 * @param void No parameters.
 * @return void
 */
static void run_monitor_tests(void) {
    ...
    RUN_TEST(test_monitor_page_next);
    RUN_TEST(test_monitor_page_wrap);
    RUN_TEST(test_monitor_page_ignore);
    RUN_TEST(test_monitor_poll_none);
